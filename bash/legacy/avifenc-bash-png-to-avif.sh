...
# HOW TO RUN 
# ------------------------------------------------------------
# chmod +x avifenc-bash-png-to-avif.sh avifenc-png-jpg-avif.conf
#
# Safe dry-run
# bash avifenc-bash-png-to-avif.sh-n
#
# Real conversion
# bash avifenc-bash-png-to-avif.sh
#
...
EXT_OUT="avif"
EXT_IN="png"
DELETE_ORIG="false"          # Change to "true" only after you are happy with results
...
# Config script name – change this if you renamed the config file
...
# Find all PNG files
mapfile -d '' -t png_files < <(find . -type f -iname "*.${EXT_IN}" -print0)
...
log_all "Found ${#png_files[@]} .${EXT_IN} file(s)."
log_all ""
...
for input_file in "${png_files[@]}"; do
    ...
    log_success "*******************************************"
    log_success "[$TOTAL_FILES/${#png_files[@]}] Converting ${EXT_IN}: $input_file"
    ...
    # Pause in dry-run so output is readable
