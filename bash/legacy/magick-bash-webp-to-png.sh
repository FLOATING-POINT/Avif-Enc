#!/usr/bin/env bash
# magick-bash-webp-to-png.sh
# Linux version of magick-bat-webp-to-png.bat
...
# HOW TO RUN 
# ------------------------------------------------------------
# chmod +x magick-bash-webp-to-png.sh magick-bash-webp-png.conf
#
# Safe dry-run
# bash magick-bash-webp-to-png.sh -n
#
# Real conversion
# bash magick-bash-webp-to-png.sh
...
mapfile -d '' -t webp_files < <(find . -type f -iname "*.${EXT_IN}" -print0)
...
log_all "Found ${#webp_files[@]} .${EXT_IN} file(s)."
log_all ""
...
for input_file in "${webp_files[@]}"; do
    ...
    log_success "*******************************************"
    log_success "[$TOTAL_FILES/${#webp_files[@]}] Converting ${EXT_IN}: $input_file"
